            ...
            <tr>
                ...
                <td>
                    <button type="button"
                        ...
                        title="Edit"
                        hx-get="{{ route('master.employee-types.edit', $sp->id) }}"
                        hx-target="#mainModal-content"
                        hx-swap="innerHTML"
                        data-bs-toggle="modal"
                        data-bs-target="#mainModal">
                    ...
                    <button type="button"
                        ...
                        hx-delete="{{ route('master.employee-types.destroy', $sp->id) }}"
                        hx-target="closest tr"
                        hx-swap="outerHTML swap:1s"
                        hx-trigger="confirmed"
                        @click="confirmDelete($el, '{{ addslashes($sp->nama) }}')">
...
@if(!$jenisPegawais->isEmpty())
<div class="mt-3">
    {{ $jenisPegawais->appends(['search' => request('search'), 'per_page' => request('per_page')])->onEachSide(1)->links('pagination::bootstrap-5') }}
</div>
@endif