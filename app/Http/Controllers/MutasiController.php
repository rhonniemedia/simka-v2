...
class MutasiController extends Controller
{
    ...
    public function index(Request $request)
    {
        $query = Pegawai::with(['statusPegawai', 'jenisPegawai', 'jabatan', 'jurusan'])
            ->whereNotIn('status', ['aktif', 'draft', 'pensiun'])
        ...
        if ($request->header('HX-Request')) {
            return view('contents.mutasi.partials.table', compact('pegawais'));
        }
        ...
        return view('contents.mutasi.index', compact(
            'pegawais',
            'statusPegawais',
            'jenisPegawais',
            'jabatans',
    ...
    public function create()
    {
        ...
        return view('contents.mutasi.partials.form', [
            'mutation' => new Pegawai(), // Objek kosong
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'status'     => 'required|in:pindah,mundur,pensiun,meninggal',
            ...
            return $this->successResponse('mutasiSaved', 'Status mutasi berhasil diperbarui.');
        } catch (ValidationException $e) {
            // Menggunakan trait untuk render ulang form saat validasi gagal
            return $this->validationErrorResponse(
                new Pegawai(),
                $e,
                'contents.mutasi.partials.form',
                'mutation',
                ['daftarPegawais' => $daftarPegawais]
            );
        }
    ...
    {
        $mutation = Pegawai::findOrFail($id);
        $daftarPegawais = Pegawai::select('name', 'peg_slug')->get();
